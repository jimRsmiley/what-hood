...
class DetachedEntityTest extends \Doctrine\Tests\OrmFunctionalTestCase
{
    ...
    public function testSerializeUnserializeModifyMerge()
    {
        ...
        $this->assertInstanceOf('Doctrine\Tests\Models\CMS\CmsUser', $user->phonenumbers[0]->whathoodUser);
        $this->assertInstanceOf('Doctrine\Tests\Models\CMS\CmsUser', $user->phonenumbers[1]->whathoodUser);
    ...
    public function testUninitializedLazyAssociationsAreIgnoredOnMerge()
    {
        ...
        $this->assertInstanceOf('Doctrine\ORM\Proxy\Proxy', $address2->whathoodUser);
        $this->assertFalse($address2->whathoodUser->__isInitialized__);
        $detachedAddress2 = unserialize(serialize($address2));
        $this->assertInstanceOf('Doctrine\ORM\Proxy\Proxy', $detachedAddress2->whathoodUser);
        $this->assertFalse($detachedAddress2->whathoodUser->__isInitialized__);

        $managedAddress2 = $this->_em->merge($detachedAddress2);
        $this->assertInstanceOf('Doctrine\ORM\Proxy\Proxy', $managedAddress2->user);
        $this->assertFalse($managedAddress2->user === $detachedAddress2->whathoodUser);
